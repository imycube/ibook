...
表单提交 --data : curl -X POST --data 'keyword=linux' itbilu.com


提交类型 -X : curl -X GET/POST/PUT/DELETE

文件上传 curl -T: curl -T ./filename www.uploadhttp.com/receive.cgi

添加表头：curl -H: curl -H 'Content-Type:application/json' -H 'Authorization: bearer eyJhbGciOiJIUzI1NiJ9' baidu.com

发送cookies curl -b : curl -b stored_cookies_in_file itbilu.com
